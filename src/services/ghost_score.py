...
# ---------------------------------------------------------------------------
# Prompt (Prompt 1 from the product spec)
# ---------------------------------------------------------------------------
SYSTEM_PROMPT = """你是一个专业的招聘行业数据分析师和求职防诈骗专家。你的任务是分析给定的职位描述（Job Description）及元数据，评估该岗位是否为"Ghost Job（幽灵岗位/虚假挂单）"或"中介二次抓取防失效岗位"。
...
{
  "ghost_score": 0-100 的整数,  // 0 为极度真实，100 为极高风险 Ghost Job
  "is_ghost_job": true/false,   // ghost_score > 50 时为 true
...
class GhostVerdict:
    ...
    risk_factors: List[str] = None  # type: ignore[assignment]
    recommendation: str = "直接投递"
    one_liner: str = ""
...
def heuristic_signals(
    ...
    score = 0
    found: List[tuple] = []  # (weight, text) — sorted later so the heaviest reasons surface
    ...
    if post_age_days is not None:
        ...
        if age is not None:
            if age > 60:
                score += 50
                found.append((50, f"挂单已 {int(age)} 天，远超 60 天红线"))
            elif age > 30:
                score += 30
                found.append((30, f"挂单已 {int(age)} 天，超过 30 天警戒线"))

    # 2. Source traceability
    traceable = source_type in ("direct_ats", "greenhouse", "lever", "ashby", "workable",
                                "smartrecruiters", "workday", "breezy", "recruitee",
                                "personio", "teamtailor", "careers_page")
    if not traceable:
        score += 20
        if source_type in ("agency", "agency_scraped", "aggregator"):
            found.append((25, "疑似中介/聚合站二次抓取，无雇主直招入口"))
        else:
            found.append((20, "无明确源头 ATS 投递入口，仅平台内一键投递"))
    ...
    if not jd_text or len(jd_text.strip()) < 200:
        score += 20
        found.append((20, "JD 内容过短，缺少团队/产品/技术栈细节"))
    elif len(vague_hits) >= 2 and len(specific_hits) == 0:
        score += 20
        found.append((20, f"JD 模板化套话多（{vague_hits[0]} 等）且无具体技术栈"))

    # 4. Evergreen / pipeline wording
    evergreen_hits = [p for p in EVERGREEN_PATTERNS if p in text]
    if evergreen_hits:
        score += 30
        found.append((30, f"含常设招聘/人才库特征词：{evergreen_hits[0]}"))

    # 5. Agency wording
    agency_hits = [p for p in AGENCY_PATTERNS if p in text]
    if agency_hits:
        score += 20
        found.append((20, f"出现中介代招口吻：{agency_hits[0]}"))

    # Hard signal: the employer's own ATS already dropped it
    if source_status == "closed":
        score = max(score + 40, 95)
        found.append((100, "源头 ATS 已下架该岗位（招聘已结束）"))

    score = max(0, min(100, score))
    factors = [text_ for _, text_ in sorted(found, key=lambda item: item[0], reverse=True)]
    ...
    return {
        ...
        "risk_factors": factors[:3] or ["未发现明显幽灵岗位信号"],
        "recommendation": rec,
        "signals": {
...
def _one_liner_from_heuristic(score: int, factors: List[str], recommendation: str) -> str:
    if score > 50:
        reason = factors[0] if factors else "存在明显风险信号"
        return f"该岗位 Ghost 风险偏高（{score}/100）：{reason}。{recommendation}。"
    return f"暂未发现明显幽灵岗位特征（风险 {score}/100），可以正常准备投递。"
...
class GhostAnalyzer:
    ...
    def __init__(self, provider: Optional[str] = None, timeout: int = 45):
    ...
    def analyze(
        ...
        baseline = heuristic_signals(
            job_title, company_name, post_age_days, source_type, jd_text, source_status
        )

        # Hard, non-probabilistic signal: the source ATS already removed it.
        if source_status == "closed":
            factors = ["源头 ATS 已下架该岗位，招聘流程已结束"] + [
                f for f in baseline["risk_factors"] if "已下架" not in f
            ]
            return GhostVerdict(
        ...
        llm = self._ask_llm(
            job_title, company_name, post_age_days, source_type, jd_text, source_status, baseline
        )
        if llm:
            merged = self._merge(llm, baseline)
        ...
        return GhostVerdict(
            ...
            one_liner=_one_liner_from_heuristic(
                baseline["ghost_score"], baseline["risk_factors"], baseline["recommendation"]
            ),
            provider="heuristic",
            source_status=source_status,
        )
    ...
    def _ask_llm(
        ...
        for provider in chain:
            try:
                raw = self._call_provider(provider, user_prompt)
                if not raw:
                    continue
...
{jd or "(未提供职位描述，请主要依据元数据判断，并在 risk_factors 中说明信息不足)"}"""
    ...
    def _call_provider(self, provider: str, user_prompt: str) -> Optional[str]:
        import requests
    ...
    @staticmethod
    def _merge(llm: Dict[str, Any], baseline: Dict[str, Any]) -> GhostVerdict:
        try:
        ...
        recommendation = str(llm.get("recommendation") or baseline["recommendation"]).strip()
        one_liner = str(llm.get("one_liner") or "").strip()
        if not one_liner:
            one_liner = _one_liner_from_heuristic(score, factors, recommendation)
