...
### 1. 基础设施与自动化
*   **邮件提醒自动化**: 将 `scripts/send_alerts.py` 集成至 GitHub Actions，实现每 6 小时自动向订阅用户发送新职位提醒。
*   **API 频率限制 (Rate Limiting)**: 针对 Auth (注册/登录/重置密码) 和 Feedback 接口增加 IP 级别的频率限制，防止恶意攻击。

### 2. 用户反馈系统 (Feedback System)
*   **前端入口**: 在页面显著位置（如 Footer 或侧边栏）增加 "Feedback" 按钮，弹出反馈表单。
*   **后端存储**: 新增 `feedbacks` 集合，存储用户的意见、邮箱和提交时间。
*   **后台管理**: 提供一个受保护的 `/admin/feedback.html` 页面（或 Admin 接口），供管理员查阅所有反馈。

### 3. SEO 与品牌
*   **Meta Tags**: 添加描述、关键词和 OpenGraph 标签，确保在社交媒体分享时有精美预览。
*   **Favicon**: 配置网站图标，提升品牌辨识度。
*   **Legal Docs**: 增加基础的隐私政策和使用条款链接。
